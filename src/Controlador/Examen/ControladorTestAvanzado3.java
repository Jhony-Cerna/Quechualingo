...
import Vista.Examen.TestAvanzado3;
...
public class ControladorTestAvanzado3 implements ActionListener{
    
    TestAvanzado3 form;
    
    public ControladorTestAvanzado3(TestAvanzado3 ventana) {
        form = ventana;
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == form.btnContinuar) {
            ...
            if(form.txtPregunta2.getText().toUpperCase().trim().equals("BRAZO")){
            puntos++;
            }else puntosperdidos++;
            if(form.txtPregunta3.getText().toUpperCase().trim().equals("PIERNA")){
            puntos++;
            }else puntosperdidos++;
            if(form.txtPregunta4.getText().toUpperCase().trim().equals("BOCA")){
            puntos++;
            }else puntosperdidos++;
            if(form.txtPregunta5.getText().toUpperCase().trim().equals("CHANKA")){
            puntos++;
            }else puntosperdidos++;
     ...
     private void addDocumentListener(JTextField textField, JProgressBar jpbProgreso) {
        textField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            private void updateProgressBar() {
                int textFieldCount = 5; //Cantidad de respuestas en el ejercicio
                int emptyFields = 0;
                ...
                double progressBarValue = (textFieldCount - emptyFields) * 20; // Aumentar un 25% por cada campo q no esta vacío
